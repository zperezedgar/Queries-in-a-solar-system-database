...
/*we look for the lightest and the heaviest planets in the solar system*/
SELECT body AS Heaviest_Planet, MAX(mass)  AS mass FROM solar_system_objects WHERE type_of_object LIKE "%planet (%";
SELECT body AS Lightest_Planet, MIN(mass) AS mass FROM solar_system_objects WHERE type_of_object LIKE "%planet (%";

/*which is the total mass of the solar system?*/
SELECT SUM(mass) AS total_mass_system FROM solar_system_objects;
/*which is the total mass of the solar system relative to the earth?*/
SELECT SUM(mass_rel) AS relative_total_mass_system FROM solar_system_objects;

/*find a planet with similar gravity and radius that the earth*/
SELECT body AS earth_like_planet, surface_gravity_rel, mean_radius_rel FROM solar_system_objects WHERE surface_gravity_rel<1.6 AND surface_gravity_rel>.4 AND mean_radius_rel<1.6 AND mean_radius_rel>0.6 AND body!="Earth";

/*which is the largest satellite?*/
SELECT body AS largest_satellite, MAX(mean_radius) AS mean_radius, type_of_object FROM solar_system_objects
WHERE type_of_object LIKE "%satellite%";

/*which is the smallest satellite?*/
SELECT body AS smallest_satellite, MIN(mean_radius) AS mean_radius, type_of_object FROM solar_system_objects
WHERE type_of_object LIKE "%satellite%";

/*which is the total volume of the dwarf planets?*/
SELECT SUM(volume) AS volume_dwarf_planets FROM solar_system_objects WHERE type_of_object LIKE "%dwarf%";

/*how many objects in the solar system have a round shape?*/
SELECT COUNT(body) AS number_round_bodies FROM solar_system_objects WHERE shape="round";

/*group the objects by its shape and show the sets for which the sum of the relative volumes whithin the set is less than 1 and greater than 0.001*/
SELECT shape, SUM(volume_rel) AS total_volume_rel FROM solar_system_objects GROUP BY shape HAVING SUM(volume_rel)<1 AND SUM(volume_rel)>0.001;
